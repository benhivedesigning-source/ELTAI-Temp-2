                    ...
                    <div className="absolute left-[2rem] md:left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-20">
                      {node.isHero ? (
                        ...
                        <div className="relative group-hover:scale-150 transition-transform duration-500">
                          <div
                            className={`absolute inset-0 rounded-full ${node.dotColor} blur-sm opacity-30`}
                          ></div>
                          <div
                            className={`w-5 h-5 rounded-full ${node.dotColor} border-[3px] border-white relative z-10 shadow-md`}
                          />
                        </div>
                      )}
                    </div>
                    ...
                    >
                      ...
                      >
                        <div className="relative group/card">
                          ...
                          <div
                            className={`absolute -inset-[2px] rounded-[2rem] bg-gradient-to-r from-[#e76366] to-[#8355eb] opacity-0 group-hover/card:opacity-100 transition-opacity duration-500 blur-sm`}
                          ></div>

                          <div
                            className={`relative bg-gray-50 border border-gray-100 transition-all duration-500 ${node.isHero ? "group-hover/card:border-[#e76366] group-hover/card:ring-1 group-hover/card:ring-[#e76366]/20" : ""} rounded-[2rem] p-8 ${node.hoverBorder} ${node.glow} hover:bg-white hover:-translate-y-2 overflow-hidden cursor-pointer shadow-sm`}
                          >
                            {/* Watermark Number */}
                            <div
                              className={`absolute -right-4 -bottom-6 text-[8rem] font-black pointer-events-none font-['Poppins'] leading-none select-none transition-transform duration-500 group-hover/card:scale-110 text-gray-900/[0.03] ${node.isHero ? "group-hover/card:text-[#e76366]/[0.06]" : "group-hover/card:text-gray-900/[0.05]"}`}
                            >
                            ...
                            <div className="flex items-center gap-4 mb-6 relative z-10">
                              <div
                                className={`w-14 h-14 rounded-2xl ${node.pillBg} flex items-center justify-center shadow-md shrink-0 group-hover/card:scale-110 transition-transform duration-500`}
                              >
                              ...
                              <div className="flex-1">
                                ...
                                <div className="font-['Poppins'] font-bold text-gray-900 text-lg leading-tight">
                                  <Ordinal text={node.date} />
                                </div>
                              </div>
                            </div>
                            ...
                            <div className="relative z-10">
                              <h3
                                className={`font-['Poppins'] font-black text-2xl leading-snug mb-4 transition-all text-gray-900 group-hover/card:text-transparent group-hover/card:bg-clip-text group-hover/card:bg-gradient-to-r group-hover/card:from-[#e76366] group-hover/card:to-[#8355eb]`}
                              >
                              ...
                              {node.badge && (
                                <div
                                  className={`inline-flex items-center gap-2 border rounded-xl px-4 py-2 text-[11px] font-bold mb-4 ${node.badge.colorClass.replace("bg-amber-950", "bg-amber-50").replace("bg-green-950", "bg-green-50").replace("text-amber-400", "text-amber-700").replace("text-green-400", "text-green-700")} shadow-inner`}
                                >
                                  {node.badge.icon && (
                                    <node.badge.icon size={13} />
                                  )}
                                  {node.badge.text}
                                </div>
                              )}
